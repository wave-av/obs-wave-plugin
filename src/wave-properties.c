...
obs_properties_t *wave_output_properties(void *unused)
{
	...
	obs_properties_add_text(props, "gateway_url", obs_module_text("WAVE.GatewayURL"), OBS_TEXT_DEFAULT);

	obs_properties_add_text(props, "stream_key", obs_module_text("WAVE.StreamKey"), OBS_TEXT_PASSWORD);

	obs_property_t *codec = obs_properties_add_list(props, "codec", obs_module_text("WAVE.Codec"),
...
{
	obs_data_set_default_string(settings, "gateway_url", WAVE_DEFAULT_GATEWAY);
