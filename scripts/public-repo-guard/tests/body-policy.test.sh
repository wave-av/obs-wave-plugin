...
# pin their own FICTIONAL names so they are hermetic, do not depend on CI
# configuration, and do not park real internal repo names in a public tree.
export GUARD_PRIVATE_REPOS="acme-private-alpha, acme-private-beta, acme-private-gamma"
...
expect 1 'private repo + service binding' \
  'This adds a service binding from the worker to acme-private-gamma for settlement.'
...
expect 0 'bare private-repo cross-reference' \
  'This is the companion change to acme-private-beta#260; merge that one first.'
expect 0 'two private repos, no operational detail' \
  'Both acme-private-alpha and acme-private-beta will need a follow-up for this.'
...
expect 0 'lowercase identifier near a private repo is not a credential name' \
  'acme-private-alpha now derives the cache_key from the room id.'
...
expect 0 'explicit guard:allow with a reason' \
  'Example for the docs: acme-private-alpha holds EXAMPLE_SECRET — guard:allow documented-example'
