...
- `public-repo-guard` now scans PR/issue/comment BODIES, not just the tree:
  ...
  `pull_request_review_comment`, `pull_request_review`, and PR `edited`) runs
  the new vendored `scripts/public-repo-guard/body-policy.sh` (+ its fixture
  self-test) against the event's title/body text. Fail-closed: a broken
  scanner is a red run, never a silent pass — including an empty
  `GUARD_PRIVATE_REPOS` variable anywhere it must be set (fork PRs, where
  GitHub withholds configuration variables, skip that one rule with a visible
  warning instead).
